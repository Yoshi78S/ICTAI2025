...
class Model(nn.Module):
	def __init__(self, image_embedding, text_embedding, audio_embedding=None):
		...
		if audio_embedding != None:
			self.individual_modal_weight = nn.Parameter(torch.Tensor([0.3333, 0.3333, 0.3333]* (args.user + args.item)))
			#self.modal_weight = nn.Parameter(torch.Tensor([0.3333, 0.3333, 0.3333]))
		else:
			self.individual_modal_weight = nn.Parameter(torch.Tensor([0.5, 0.5]* (args.user + args.item)))
	...
	def forward_MM(self, adj, image_adj, text_adj, audio_adj=None):
		...
		individual_weight = self.softmax2(self.individual_modal_weight.view(args.user + args.item, -1))
		...
		if audio_adj is None:
			embedsModal = individual_weight[:, 0].unsqueeze(1) * embedsImage + individual_weight[:, 1].unsqueeze(1) * embedsText
		else:
			embedsModal = individual_weight[:, 0].unsqueeze(1) * embedsImage + individual_weight[:, 1].unsqueeze(1) * embedsText + individual_weight[:, 2].unsqueeze(1) * embedsAudio
		"""
		if audio_adj == None:
	...
	def computer_original(self, graph, mask):
		users_emb = self.uEmbeds
		items_emb = self.iEmbeds
		all_emb = torch.cat([users_emb, items_emb])
		all_emb = all_emb * (1 - mask)
		embs = [all_emb]

		g_dropped = graph

		for layers in range(args.gnn_layer):
			all_emb = torch.sparse.mm(g_dropped, all_emb)
			embs.append(all_emb)
		embs = torch.stack(embs, dim=1)
		light_out = torch.mean(embs, dim=1)
		users1, items1 = torch.split(light_out, [args.user, args.item])
		return users1, items1
